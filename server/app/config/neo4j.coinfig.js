...
session.run ('MATCH (c:Category {name: "Visas"}) RETURN c')
  .then(function(results){
      if(results.records.length == 0){
        const categoryNames = ["Accomodation", "Visas", "City_Office", "Flights", "Universities" ];
        categoryNames.forEach(function addCategory(categoryName){
            const session = driver.session();
            session
            .run('CREATE (c:Category {name: $categoryName})', { categoryName: categoryName })
            .then(() => {
              session.close(() => { 
